...
/// The last reported window title, if any.
window_title: std.ArrayList(u8),
...
pub fn deinit(self: *Terminal, alloc: Allocator) void {
    ...
    self.window_title.deinit(alloc);
    self.* = undefined;
}
...
pub fn setWindowTitle(self: *Terminal, title: []const u8) !void {
    self.window_title.clearRetainingCapacity();
    try self.window_title.appendSlice(self.gpa(), title);
}

pub fn windowTitle(self: *const Terminal) []const u8 {
    return self.window_title.items;
}
