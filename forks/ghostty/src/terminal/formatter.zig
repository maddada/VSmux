...
pub const TerminalFormatter = struct {
    ...
    pub const Extra = packed struct {
        ...
        /// Emit the current window title using OSC 2.
        title: bool,
    ...
    pub fn format(
        ...
        if (self.opts.emit == .vt) {
            ...
            // Emit the current window title using OSC 2.
            if (self.extra.title) {
                const title = self.terminal.windowTitle();
...
test "Terminal vt with title" {
    ...
    s.nextSlice("\x1b]2;My Test Title\x1b\\hello");
    ...
    try testing.expectEqualStrings(t.windowTitle(), t2.windowTitle());
}
