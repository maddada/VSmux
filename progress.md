...
- Verification:
  - added `VSmux.backgroundSessionTimeoutMinutes` with a default of `0` for no expiry
...
- Status: completed
- Goal: make the VSmux sessions surface appear in the right-side secondary sidebar by default instead of the left activity bar.
- Verification:
  - the extension now contributes its custom view container through `contributes.viewsContainers.secondarySidebar`
  - the existing `VSmuxSessions` container id and `VSmux.sessions` view id stay unchanged, so reveal commands continue to target the same view container
