...
import {
  ...
  s,
  t as c,
} from "./sidebar-story-meta-Dzquq7O5.js";
async function l() {
  await v(() => g(a().some((e) => e.type === `ready`)).toBe(!0));
}
async function u(e) {
  await v(() => g(a().some((t) => h(t, e))).toBe(!0));
}
async function d(e, t, n) {
  let r = f(t),
    i = f(n);
  await e.pointer([
    { coords: r, keys: `[MouseLeft>]`, target: t },
    { coords: i, target: n },
    { coords: i, keys: `[/MouseLeft]`, target: n },
  ]);
}
function f(e) {
...
async function p(e) {
  let t = e.getBoundingClientRect();
  await _.contextMenu(e, { bubbles: !0, clientX: t.left + t.width / 2, clientY: t.top + 12 });
}
function m(e) {
  if (!(e instanceof HTMLElement)) throw Error(`Expected HTMLElement selector to resolve.`);
  return e;
}
function h(e, t) {
...
e(() => {
  (s(),
    r(),
    ({ expect: g, fireEvent: _, waitFor: v, within: y } = __STORYBOOK_MODULE_TEST__),
    (b = { title: `Sidebar/Interactions`, args: c, argTypes: i, decorators: o, render: t }),
    (x = {
      args: { highlightedVisibleCount: 4, visibleCount: 4 },
      play: async ({ canvas: e, step: t, userEvent: r }) => {
        (await l(),
          n(),
          await t(`request a new session`, async () => {
            (await r.click(e.getByRole(`button`, { name: `New Session` })),
              await u({ type: `createSession` }));
          }),
          await t(`change sessions shown`, async () => {
            (n(),
              await r.click(e.getByRole(`button`, { name: `Show 6 sessions` })),
              await u({ type: `setVisibleCount`, visibleCount: 6 }));
          }),
          await t(`switch layout mode`, async () => {
            (n(),
              await r.click(e.getByRole(`button`, { name: `Horizontal` })),
              await u({ type: `setViewMode`, viewMode: `horizontal` }));
          }),
          await t(`open sidebar settings`, async () => {
            (n(),
              await r.click(e.getByRole(`button`, { name: `Open sidebar theme settings` })),
              await u({ type: `openSettings` }));
          }));
      },
    }),
    (S = {
      play: async ({ canvas: e, canvasElement: t, step: r, userEvent: i }) => {
        let a = y(t.ownerDocument.body);
        (await l(),
          n(),
          await r(`focus a session from its card`, async () => {
            (await i.click(e.getByRole(`button`, { name: /Harbor Vale/i })),
              await u({ sessionId: `session-3`, type: `focusSession` }));
          }),
          await r(`rename through the session context menu`, async () => {
            (n(),
              await p(e.getByRole(`button`, { name: /Harbor Vale/i })),
              await i.click(await a.findByRole(`menuitem`, { name: `Rename` })),
              await u({ sessionId: `session-3`, type: `promptRenameSession` }));
          }),
          await r(`terminate through the session context menu`, async () => {
            (n(),
              await p(e.getByRole(`button`, { name: /Harbor Vale/i })),
              await i.click(await a.findByRole(`menuitem`, { name: `Terminate` })),
    ...
    (C = {
      play: async ({ canvas: e, canvasElement: t, step: r, userEvent: i }) => {
        ...
        (await l(),
          n(),
          await r(`reorder sessions inside a group`, async () => {
            (await d(
              i,
              m(a.querySelector(`[data-sidebar-session-id="session-1"]`)),
              m(a.querySelector(`[data-sidebar-session-id="session-2"]`)),
            ),
              await u({
                groupId: `group-1`,
    ...
    (w = {
      play: async ({ canvasElement: e, step: t, userEvent: r }) => {
        let i = e.ownerDocument;
        (await l(),
          n(),
          await t(`move a session into another group`, async () => {
            (await d(
              r,
              m(i.querySelector(`[data-sidebar-session-id="session-3"]`)),
              m(i.querySelector(`[data-sidebar-group-id="group-2"]`)),
            ),
              await u({ groupId: `group-2`, sessionId: `session-3`, type: `moveSessionToGroup` }));
          }));
      },
    }),
    (x.parameters = {
...
export {
  w as DragToMoveAcrossGroups,
  C as DragToReorderWithinGroup,
  S as SessionCardActions,
  x as ToolbarActions,
