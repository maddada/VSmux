...
- `media/VSmux.svg`: activity-bar icon
...
3. In the Extension Development Host, run `VSmux: Open Workspace`.
...
- `VSmux.backgroundSessionTimeoutMinutes` controls how long the detached daemon keeps sessions alive after the last VS Code client disconnects. `0` disables expiry.
