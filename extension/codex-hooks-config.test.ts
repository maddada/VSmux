...
describe("getCodexHookCommand", () => {
  test("should force Node mode for non-Windows Codex hook commands", () => {
    expect(getCodexHookCommand("/tmp/vsmux-notify.js", "/usr/local/bin/node", "linux")).toBe(
      "ELECTRON_RUN_AS_NODE=1 '/usr/local/bin/node' '/tmp/vsmux-notify.js'",
    );
  });
});

describe("mergeCodexHookSettingsContent", () => {
  test("should append the VSmux hook without removing existing hooks", () => {
  ...
  test("should avoid duplicating the same VSmux hook command", () => {
  ...
  test("should replace stale VSmux commands and remove the old VSmux Stop hook", () => {
    const mergedContent = mergeCodexHookSettingsContent(
      JSON.stringify(
        {
          hooks: {
            Stop: [
              {
                hooks: [
                  {
                    command: "'/usr/local/bin/node' '/tmp/vsmux-notify.js'",
                    type: "command",
                  },
                ],
              },
