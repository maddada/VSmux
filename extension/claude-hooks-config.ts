...
export function mergeClaudeHookSettingsContent(
  ...
  const vsmuxSettings = JSON.parse(
  ...
  for (const eventName of CLAUDE_HOOK_EVENT_NAMES) {
    const desiredGroups = getMatcherGroups(vsmuxSettings.hooks?.[eventName]);
...
function isVsmuxClaudeHookCommand(command: string | undefined, notifyCommandPath: string): boolean {
  ...
  return /maddada\.vsmux[/\\]terminal-host-daemon[/\\]agent-shell-integration[/\\]hooks[/\\]claude[/\\]notify(?:\.sh|\.cmd)?/.test(
    command,
  );
}
